...
class ActionsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: const [TextButton(onPressed: Transaction.clear, child: Text("Clear", style: TextStyle(color: Colors.white)))],
      ),
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            if (Platform.isIOS) button("APPLE_PAY", applePay),
          ],
        ),
      ),
    );
  ...
  saleWithCard() {
    final order = EdfaPgSaleOrder(
        ...
        currency: "SAR",
        amount: 0.10 //Random().nextInt(9)/10, // will not exceed 0.9
        );
    ...
    final payer = EdfaPgPayer(
        ...
        phone: "+966500409598",
        ip: "66.249.64.248",
        options: EdfaPgPayerOption(
            // Options
            middleName: "Muhammad Iqbal",
            birthdate: DateTime.parse("1987-03-30"),
            address2: "King Fahad Road",
            state: "Olaya"));

    EdfaCardPay().setOrder(order).setPayer(payer).onTransactionSuccess((response) {
  ...
  applePay() async {
    ...
    EdfaApplePay().setOrder(order).setPayer(payer).setApplePayMerchantID("merchant.expresspay.darlana").onAuthentication((response) {
